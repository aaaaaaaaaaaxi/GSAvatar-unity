// NpzAvatarLoader.cs — runtime NPZ → WebAvatarAsset conversion.
//
// Mirrors Editor/AvatarDataBuilder.cs (which is in the editor assembly
// and therefore unavailable in builds) so the same .npz files produced
// by the webavatar-rust pipeline can be loaded at runtime via
// NpzAvatarLoader.Load(path) — for example from a runtime file picker,
// Application.streamingAssetsPath, or a download cache.
//
// Calculation rules (bounds, SH band-0 DC offset, weight packing,
// MLP weight/bias permutation, basis layout) are intentionally kept
// bit-identical to AvatarDataBuilder so a file imported via the asset
// DB at edit time and a file loaded via this class at runtime produce
// the same WebAvatarAsset content.
...
namespace WebAvatar
{
    public static class NpzAvatarLoader
    {
        ...
        // Resolve a path on disk and convert it.  Absolute paths are
        // used verbatim; relative paths are resolved against
        // Application.streamingAssetsPath.  Throws on any I/O or
        // schema error.
        public static WebAvatarAsset Load(string npzPath)
        {
            if (string.IsNullOrEmpty(npzPath))
                throw new LoadException("NPZ path is empty");
            if (!System.IO.File.Exists(npzPath))
                throw new LoadException("NPZ file not found: " + npzPath);

            using var archive = NpzArchive.Open(npzPath);
            return Build(archive);
        }

        // Build a populated WebAvatarAsset from an open NPZ archive.
        // Pure function with respect to `archive` (no asset DB calls,
        // no editor-only APIs), so it works the same in the editor and
